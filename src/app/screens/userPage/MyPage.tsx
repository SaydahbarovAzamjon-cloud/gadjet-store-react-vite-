...
export default function MyPage() {
  ...
  // ── Profile state ─────────────────────────────────────────────────────────────
  // FIX 1: Default hardcoded URL olib tashlandi. null = harf avatar ko'rsatiladi.
  ...
  // FIX 2: hasInitialized ref — forma faqat BIR marta (birinchi user load da) to'ldiriladi.
  // verifyAuth() dan keyin user o'zgarganda forma qayta overwrite bo'lmaydi.
  ...
  useEffect(() => {
    if (user && !hasInitialized.current) {
      ...
      // Faqat haqiqiy rasm mavjud bo'lsa o'rnatiladi
  ...
  // ── Payment state — localStorage da saqlanadi (refresh dan keyin saqlanib qoladi) ──
  const CARDS_KEY = "gadjet_payment_cards";

  const [paymentCards, setPaymentCards] = useState<PaymentCard[]>(() => {
    try {
      const saved = localStorage.getItem(CARDS_KEY);
      if (saved) return JSON.parse(saved) as PaymentCard[];
    } catch {}
    // Birinchi marta: default karta
    return [{ id: 1, type: "Credit Card", cardNumber: "4242", expiry: "12/25", isDefault: true }];
  });

  // Kartalar o'zgarganda localStorage ga yozish
  useEffect(() => {
    try {
      localStorage.setItem(CARDS_KEY, JSON.stringify(paymentCards));
    } catch {}
  }, [paymentCards]);
  ...
  const handleSave = async () => {
    try {
      ...
      // verifyAuth keyin hasInitialized true bo'lgani uchun formani qayta reset qilmaydi
  ...
  const handleAddressSave = async () => {
    try {
      ...
      await dispatch(verifyAuth());
      // Address tab uchun local state ham yangilash
      setAddress(editAddressValue.trim());
  ...
  const handleCardSave = (id: number) => {
    ...
    setPaymentCards((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, cardNumber: last4, expiry: editCardExpiry } : c
      )
    );
                  ...
                  <div className="bg-card rounded-lg border border-border p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        {user?.memberAddress ? (
                          <span className="text-xs bg-accent/10 text-accent px-2 py-1 rounded mt-1 inline-block">Default</span>
                        ) : (
                          <span className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded mt-1 inline-block">No address set</span>
                        )}
                      </div>
                      {!isEditingAddress && (
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" className="border-border gap-1" onClick={handleAddressEditOpen}>
                    ...
                    {isEditingAddress ? (
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2 pt-1">
                          <Button size="sm" onClick={handleAddressSave} disabled={addressSaving}
                      ...
                      <div>
                        ...
                        {user?.memberAddress ? (
                          ...
                          <p className="text-muted-foreground italic text-sm">No address added yet. Click Edit to add one.</p>
                        )}
                      </div>
                    )}
                  </div>
                    ...
                    <div key={card.id} className="bg-card rounded-lg border border-border p-6">
                      {editingCardId === card.id ? (
                        <div className="space-y-4">
                          ...
                          <div>
                            <label className="text-xs text-muted-foreground block mb-1">Last 4 digits of card number</label>
                          ...
                          <div>
                            <label className="text-xs text-muted-foreground block mb-1">Expiry Date (MM/YY)</label>
                            <input
                              type="text"
                          ...
                          <div className="flex gap-2 pt-1">
                            <Button size="sm" onClick={() => handleCardSave(card.id)}
                              className="bg-gradient-to-r from-blue-500 to-purple-600 text-white gap-1">
                              <Check className="w-3 h-3" />Save
                            </Button>
                            <Button size="sm" variant="outline" className="border-border gap-1" onClick={() => setEditingCardId(null)}>
                              <X className="w-3 h-3" />Cancel
                            </Button>
                          </div>
                        </div>
                        ...
                        <>
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                              <Button size="sm" variant="outline" className="border-border gap-1" onClick={() => handleCardEditOpen(card)}>
                                <Pencil className="w-3 h-3" />Edit
                              </Button>
                              <Button size="sm" variant="outline" className="border-border text-destructive gap-1" onClick={() => handleCardDelete(card.id)}>
                                <Trash2 className="w-3 h-3" />Delete
                              </Button>
                            </div>
                          </div>
                          <p className="text-foreground font-mono tracking-widest text-lg">**** **** **** {card.cardNumber}</p>
                          <p className="text-muted-foreground text-sm mt-2">Expires: {card.expiry}</p>
                        </>
                      )}
                    </div>
                  ))}
